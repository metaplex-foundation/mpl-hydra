...
# External program/account IDs to fetch from mainnet
EXTERNAL_ID=(
    "metaqbxxUerdq28cj1RbAWkYQm3ybzjb6a8bt518x1s"  # Token Metadata
    "SysExL2WDyJi9aRZrXorrjHJut3JwHQ7R9bTyctbNNG"  # System Extras
    "TokExjvjJmhKaRBShsBAsbSvEWMA1AgUNK7ps4SAc2p"  # Token Extras
)

# Output filenames (must match order of EXTERNAL_ID)
EXTERNAL_SO=(
...
copy_from_chain() {
    ...
    case "$ACCOUNT_TYPE" in
        "bin")
            solana account -u $RPC ${EXTERNAL_ID[$i]} -o ${OUTPUT}/${PREFIX}$1 > /dev/null
        ;;
        "so")
            solana program dump -u $RPC ${EXTERNAL_ID[$i]} ${OUTPUT}/${PREFIX}$1 > /dev/null
        ;;
        *)
    ...
    if [ -z "$PREFIX" ]; then
        echo "Wrote account data to ${OUTPUT}/${PREFIX}$1"
    fi
}
